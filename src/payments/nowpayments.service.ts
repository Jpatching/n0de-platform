...
export class NOWPaymentsService {
  ...
  async createPayment(payment: any) {
    try {
      ...
      this.logger.error(`Failed to create NOWPayments payment: ${error.message}`);
      throw error;
    }
  }
  ...
  async getAvailableCurrencies() {
    try {
      ...
      this.logger.error(`Failed to get NOWPayments currencies: ${error.message}`);
      throw error;
    }
  }
  ...
  async listPayments(limit = 10, page = 0, sortBy = 'created_at', orderBy = 'desc') {
    try {
      const params = new URLSearchParams({
      ...
      this.logger.error(`Failed to list NOWPayments payments: ${error.message}`);
      throw error;
    }
  }
}