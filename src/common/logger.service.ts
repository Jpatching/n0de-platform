...
export class LoggerService implements NestLoggerService {
  ...
  private getColorCode(level: string): string {
    switch (level) {
      ...
      case 'verbose': return '\x1b[37m'; // White
  ...
  logApiRequest(method: string, url: string, userId?: string, apiKeyId?: string, duration?: number) {
    this.log({
      type: 'api_request',
      method,
      url,
  ...
  logApiError(method: string, url: string, error: any, userId?: string, apiKeyId?: string) {
    this.error({
      type: 'api_error',
      method,
      url,
  ...
  logRpcCall(method: string, params: any, duration: number, success: boolean, userId?: string, apiKeyId?: string) {
    this.log({
      type: 'rpc_call',
      method,
      params,
  ...
  logSecurityEvent(event: string, details: any, userId?: string, ipAddress?: string) {
    this.warn({
      type: 'security_event',
      event,
      details,
  ...
  logPerformanceMetric(metric: string, value: number, unit: string, region?: string) {
    this.log({
