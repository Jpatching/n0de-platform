...
import { UserRole, SubscriptionType, SupportTicketStatus, PaymentStatus } from '@prisma/client';
...
export class AdminController {
  ...
  async updateUserSubscription(
    @Request() req,
    @Param('id') userId: string,
    @Body('planType', new ParseEnumPipe(SubscriptionType)) planType: SubscriptionType,
  ) {
    return this.adminService.updateUserSubscription(req.user.userId, userId, planType);
  }
  ...
  async updateTicketStatus(
    ...
    @Body('status', new ParseEnumPipe(SupportTicketStatus)) status: SupportTicketStatus,
    @Body('assignedToEmail') assignedToEmail?: string,
  ) {
    return this.adminService.updateTicketStatus(req.user.userId, ticketId, status, assignedToEmail);
  }
  ...
  @ApiQuery({ name: 'page', required: false, type: Number, description: 'Page number' })
  @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page' })
  @ApiQuery({ name: 'adminUserId', required: false, type: String, description: 'Filter by admin user ID' })
