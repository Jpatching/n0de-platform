...
export class ApiKeysController {
  ...
  async getApiKeyInsights(@Request() req, @Param('id') keyId: string) {
    return this.intelligentApiKeysService.getApiKeyInsights(req.user.userId, keyId);
  }

  @Post(':id/optimize')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Auto-optimize API key rate limiting based on AI analysis' })
  @ApiResponse({ 
    status: 200, 
  ...
  async optimizeApiKey(@Request() req, @Param('id') keyId: string) {
    return this.intelligentApiKeysService.optimizeApiKeyRateLimit(req.user.userId, keyId);
  }
  ...
  async getIntelligenceDashboard(@Request() req) {
    ...
    const insights = await Promise.all(
      userKeys.map(key => this.intelligentApiKeysService.getApiKeyInsights(req.user.userId, key.id))
    );
    
    const overview = {
      totalKeys: userKeys.length,
      activeKeys: userKeys.filter(key => key.isActive).length,
      overallHealthScore: insights.length > 0 ? 
        insights.reduce((sum, insight) => sum + ((insight as any).performance?.healthScore || 100), 0) / insights.length : 100,
      totalAnomalies: insights.reduce((sum, insight) => sum + ((insight as any).security?.anomaliesDetected || 0), 0),
      optimizationOpportunities: insights.filter(insight => 
        (insight as any).optimization?.rateLimit?.recommended !== (insight as any).optimization?.rateLimit?.current
      ).length,
    };

    const recommendations = insights.flatMap(insight => (insight as any).optimization?.performanceRecommendations || []);
    const alerts = insights.flatMap(insight => (insight as any).security?.securityEvents || []);
    ...
    return [
      {
        ...
        value: insights.length > 0 ? 
          insights.reduce((sum, insight) => sum + insight.performance.healthScore, 0) / insights.length : 100,
        trend: 'stable',
        change: 0,
      },
      {
        metric: 'Total Requests',
        value: insights.reduce((sum, insight) => sum + insight.usage.totalRequests, 0),
        trend: 'up',
        change: 15.3,
      },
      {
        ...
        value: insights.length > 0 ?
          insights.reduce((sum, insight) => sum + insight.usage.successRate, 0) / insights.length : 100,
        trend: 'up',
        change: 2.1,
      },
    ];
