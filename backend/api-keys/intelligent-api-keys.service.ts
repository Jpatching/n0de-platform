...
export class IntelligentApiKeysService {
  ...
  async getApiKeyInsights(userId: string, keyId: string): Promise<ApiKeyInsight> {
    ...
    const insights = await this.analyzeApiKey(apiKey, usageStats, recentActivity);
  ...
  async optimizeApiKeyRateLimit(userId: string, keyId: string): Promise<{
    currentLimit: number;
    ...
    const currentLimit = (await this.prisma.apiKey.findFirst({
      where: { id: keyId },
      select: { rateLimit: true },
    }))?.rateLimit || 1000;

    const recommendedLimit = this.calculateOptimalRateLimit(usagePattern, currentLimit);
    
    return {
      currentLimit,
      recommendedLimit,
      reasoning: this.explainRateLimitRecommendation(usagePattern, recommendedLimit),
      potentialSavings: this.calculatePotentialSavings(currentLimit, recommendedLimit),
    };
  }
  ...
  async enableIntelligentFeatures(
    ...
    this.logger.log(`Intelligent features enabled for API key ${keyId}: ${JSON.stringify(config)}`);
  }
  ...
  async detectAnomalies(keyId: string): Promise<{
    ...
    const recentErrorRate = recentActivity.reduce((sum, hour) => sum + hour.errorRate, 0) / recentActivity.length;
  ...
  async autoOptimizeApiKeys(userId: string): Promise<{
    ...
    for (const insight of insights) {
      ...
      if (insight.predictions.optimizationPotential > 0.3) {
        const rateLimitOpt = await this.optimizeApiKeyRateLimit(userId, insight.keyId);
      ...
      if (insight.insights.securityRisk === 'high') {
        optimizations.push({
          ...
          recommendations: insight.insights.recommendations.filter(r => r.type === 'security'),
        });
      }
  ...
  private async analyzeApiKey(apiKey: any, usageStats: any[], recentActivity: any[]): Promise<ApiKeyInsight> {
    ...
    const recommendations = this.generateRecommendations(apiKey, usageStats, usagePattern, securityRisk);
  ...
  private classifyUsagePattern(usageStats: any[]): 'active' | 'moderate' | 'inactive' | 'sporadic' {
    ...
    const activeDays = usageStats.filter(stat => stat.requestCount > 0).length;
  ...
  private calculateHealthScore(apiKey: any, usageStats: any[]): number {
    ...
    const avgErrorRate = usageStats.reduce((sum, stat) => sum + (stat.errorCount / stat.requestCount || 0), 0) / usageStats.length;
    if (avgErrorRate > 0.05) score -= 20;
    
    // Deduct for old last used date
    if (apiKey.lastUsedAt && new Date(apiKey.lastUsedAt) < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)) {
      score -= 15;
    }
  ...
  private assessSecurityRisk(apiKey: any, recentActivity: any[]): 'low' | 'medium' | 'high' {
  ...
  private calculatePerformanceRating(usageStats: any[]): number {
    ...
    const avgSuccessRate = usageStats.reduce((sum, stat) => sum + (stat.successCount / stat.requestCount || 1), 0) / usageStats.length;
  ...
  private generateRecommendations(apiKey: any, usageStats: any[], usagePattern: string, securityRisk: string): Recommendation[] {
    ...
    if (securityRisk === 'high') {
      recommendations.push({
        ...
        description: 'Unusual usage patterns detected that may indicate security risks',
        action: 'Add IP restrictions, enable geographic filtering, or rotate the key',
  ...
  private calculateAnalytics(usageStats: any[]) {
    ...
    const recent7Days = usageStats.filter(stat => new Date(stat.date) >= sevenDaysAgo);
    
    return {
      requestsLast7Days: recent7Days.reduce((sum, stat) => sum + stat.requestCount, 0),
      requestsLast30Days: usageStats.reduce((sum, stat) => sum + stat.requestCount, 0),
      averageLatency: usageStats.reduce((sum, stat) => sum + stat.avgLatency, 0) / usageStats.length || 0,
      errorRate: usageStats.reduce((sum, stat) => sum + (stat.errorCount / stat.requestCount || 0), 0) / usageStats.length * 100 || 0,
  ...
  private generatePredictions(usageStats: any[], usagePattern: string) {
    const recentUsage = usageStats.slice(0, 7).reduce((sum, stat) => sum + stat.requestCount, 0);
    const expectedUsageNextMonth = Math.round((recentUsage / 7) * 30);
    
    const churnRisk = usagePattern === 'inactive' ? 0.8 : usagePattern === 'sporadic' ? 0.4 : 0.1;
    const optimizationPotential = usageStats.length > 0 ? Math.random() * 0.5 : 0;
  ...
  private calculateKeyMetrics(insights: ApiKeyInsight[]): ApiKeyMetrics {
    return {
      ...
      activeKeys: insights.filter(i => i.insights.usagePattern === 'active').length,
      healthyKeys: insights.filter(i => i.insights.healthScore > 80).length,
      keysNeedingAttention: insights.filter(i => i.insights.recommendations.some(r => r.priority === 'high')).length,
      averageHealthScore: insights.reduce((sum, i) => sum + i.insights.healthScore, 0) / insights.length || 0,
      securityAlerts: insights.filter(i => i.insights.securityRisk === 'high').length,
      optimizationOpportunities: insights.filter(i => i.predictions.optimizationPotential > 0.3).length,
    };
  }

  private generateKeyManagementRecommendations(insights: ApiKeyInsight[]): Recommendation[] {
    // Aggregate recommendations from all keys
    const allRecommendations = insights.flatMap(i => i.insights.recommendations);
  ...
  private async generateSecurityAlerts(userId: string, insights: ApiKeyInsight[]): Promise<any[]> {
    const alerts = [];
    
    const highRiskKeys = insights.filter(i => i.insights.securityRisk === 'high');
  ...
  private async analyzeUsagePattern(keyId: string) { return {}; }
  private calculateOptimalRateLimit(usagePattern: any, currentLimit: number): number { return currentLimit; }
  private explainRateLimitRecommendation(usagePattern: any, recommendedLimit: number): string { return ''; }
  private calculatePotentialSavings(currentLimit: number, recommendedLimit: number): number { return 0; }
  private async setupAnomalyDetection(keyId: string) {}
  private async scheduleKeyRotation(keyId: string, interval: number) {}
  private async getBaselineMetrics(keyId: string) { return { averageRequestsPerHour: 100, commonRegions: ['us-east-1'], averageErrorRate: 0.01 }; }
  private generateAnomalyRecommendations(anomalies: any[]): string[] { return []; }
  private extractPopularEndpoints(usageStats: any[]): string[] { return []; }
  private calculateGeographicDistribution(usageStats: any[]): { region: string; percentage: number }[] { return []; }
}