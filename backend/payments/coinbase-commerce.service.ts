...
export class CoinbaseCommerceService {
  ...
  verifyWebhookSignature(payload: any, signature: string): boolean {
    ...
    if (!signature) {
      this.logger.warn("No signature provided for Coinbase webhook");
      return false;
    }

    if (!this.webhookSecret) {
      this.logger.error("COINBASE_COMMERCE_WEBHOOK_SECRET not configured!");
      return false;
    }
    ...
    try {
      ...
      if (isValid) {
        this.logger.log("✓ Coinbase webhook signature verified successfully");
      } else {
        this.logger.warn("✗ Coinbase webhook signature verification failed");
        this.logger.debug(
          `Expected signature prefix: ${computedSignature.substring(0, 10)}...`,
        );
        this.logger.debug(
          `Received signature prefix: ${signature.substring(0, 10)}...`,
        );
      }
