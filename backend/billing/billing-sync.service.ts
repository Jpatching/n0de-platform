...
export class BillingSyncService {
   ...
   */
  async recordUsage(userId: string, endpoint: string, computeUnits: number): Promise<void> {
    ...
    pipeline.hincrby(`usage:${userId}:${period}`, 'compute_units', computeUnits);
    ...
    pipeline.zadd(`timeline:${userId}`, timestamp, `${endpoint}:${computeUnits}`);
   ...
   */
  async handleOverage(userId: string, overageType: string, amount: number): Promise<void> {
  ...
  private async syncUserToStripe(userId: string): Promise<void> {
    ...
    const totalCalls = Object.values(usage.rpcCalls).reduce((sum, count) => sum + count, 0);
   ...
   */
  private convertToStripeUsage(usage: UsageSnapshot, subscription: any): StripeUsageRecord[] {
   ...
   */
  private async checkUsageLimits(userId: string, period: string): Promise<void> {
    ...
    if (currentUsage.requests > limits.requests * 0.8) {
      await this.sendUsageWarning(userId, 'API calls', 
        currentUsage.requests, limits.requests);
    }
    
    // Check hard limits
    if (currentUsage.requests >= limits.requests) {
      await this.handleOverage(userId, 'api_limit', 
        currentUsage.requests - limits.requests);
    }
  }
  ...
  private async enforceHardLimit(userId: string, overageType: string): Promise<void> {
    this.logger.warn(`Enforcing hard limit for user ${userId}, type: ${overageType}`);
    // Set rate limit block in Redis
    await this.redis.set(`blocked:${userId}`, overageType, 3600); // Block for 1 hour
  }

  private async handleStarterOverage(userId: string, amount: number): Promise<void> {
  ...
  private async recordBillableOverage(userId: string, overageType: string, amount: number): Promise<void> {
    ...
    this.logger.log(`Recorded billable overage: ${userId} - ${overageType}: $${amount/100}`);
  }

  private async getUsageSnapshot(userId: string): Promise<UsageSnapshot | null> {
    const period = this.getCurrentBillingPeriod();
    const usage = await this.redis.hgetall(`usage:${userId}:${period}`);
  ...
  private async markSyncComplete(userId: string, periodEnd: Date): Promise<void> {
    const key = `sync:${userId}:${this.getCurrentBillingPeriod()}`;
    await this.redis.set(key, periodEnd.toISOString(), 86400); // 24h TTL
  }

  private async sendUsageWarning(userId: string, type: string, current: number, limit: number): Promise<void> {
    this.logger.warn(`Usage warning for ${userId}: ${type} at ${current}/${limit} (${Math.round(current/limit*100)}%)`);
  ...
  async adjustUsageLimits(userId: string, newPlan: any): Promise<void> {
    this.logger.log(`Adjusting usage limits for ${userId} to plan: ${newPlan.name}`);
  ...
  async getCurrentUsage(userId: string): Promise<any> {
    try {
      ...
      const requests = await this.redis.get(`usage:${userId}:${currentPeriod}:requests`);
      const computeUnits = await this.redis.get(`usage:${userId}:${currentPeriod}:computeUnits`);
      ...
      return { requests: 0, computeUnits: 0, period: this.getCurrentPeriod(), timestamp: new Date() };
    }
  }
  ...
  private mapStripeStatus(stripeStatus: string): import('@prisma/client').SubscriptionStatus {
    const statusMap = {
    ...
    return (statusMap[stripeStatus] || 'PAST_DUE') as import('@prisma/client').SubscriptionStatus;
  }
