...
export class SubscriptionsController {
  ...
  @ApiResponse({
    status: 200,
    description: "Real-time usage stats retrieved successfully",
  })
  @ApiResponse({ status: 401, description: "Unauthorized" })
  ...
  @ApiResponse({ status: 401, description: "Unauthorized" })
  async upgradePlan(@Request() req, @Body() body: UpgradeSubscriptionDto) {
  ...
  async checkApiKeyLimit(@Request() req) {
    const canCreate = await this.subscriptionsService.checkApiKeyLimit(
      req.user.userId,
    );
    return { canCreate };
  }
