...
export class SimpleSecurityService {
  ...
  constructor(
    ...
    this.enableSecurity = this.configService.get('ENABLE_ABUSE_DETECTION') === 'true';
    this.confidenceThreshold = parseFloat(this.configService.get('ABUSE_CONFIDENCE_THRESHOLD')) || 0.95;
    
    this.logger.log(`Simple Security Service initialized - Enabled: ${this.enableSecurity}`);
  }
  ...
  async checkRequest(params: {
    ...
    try {
      ...
      const riskScore = checks.reduce((sum, check) => sum + (check.risk || 0), 0) / checks.length;
      
      // Log suspicious activity
      if (riskScore > 0.3) {
        this.logger.log(`Suspicious activity detected: IP=${params.ipAddress}, Risk=${riskScore.toFixed(2)}`);
      ...
      if (shouldBlock) {
        this.logger.warn(`HIGH RISK REQUEST BLOCKED: IP=${params.ipAddress}, Risk=${riskScore.toFixed(2)}`);
      }
  ...
  async trackRequest(params: {
    ...
    try {
      ...
      const endpointCount = await this.redisService.get(`${endpointKey}:${params.endpoint}`) || 0;
      await this.redisService.set(`${endpointKey}:${params.endpoint}`, (parseInt(endpointCount.toString()) + 1).toString());
  ...
  async getSecurityStats(): Promise<any> {
    ...
    try {
      ...
      const suspiciousIps = await this.redisService.keys('security:suspicious:*');
      const blockedRequests = await this.redisService.get('security:blocked:count') || '0';
  ...
  private async checkIpReputation(ipAddress: string): Promise<{ risk: number; reason?: string }> {
    try {
      ...
      if (parseInt(frequency.toString()) > 60) { // More than 60 requests per minute
  ...
  private async checkRequestPatterns(ipAddress: string, endpoint: string): Promise<{ risk: number; reason?: string }> {
    try {
      // Check endpoint diversity (scanning behavior)
      const endpointKeys = await this.redisService.keys(`security:endpoints:${ipAddress}:*`);
      
      if (endpointKeys.length > 10) { // Accessing many different endpoints
      ...
      if (parseInt(errors.toString()) > 20) { // High error rate
  ...
  private checkUserAgent(userAgent?: string): { risk: number; reason?: string } {
    if (!userAgent) {
    ...
    for (const pattern of suspiciousPatterns) {
      if (pattern.test(userAgent)) {
        return { risk: 0.8, reason: `Suspicious user agent: ${pattern.source}` };
      }
    }
  ...
  private async logSuspiciousActivity(ipAddress: string, riskScore: number, details: any[]): Promise<void> {
    try {
