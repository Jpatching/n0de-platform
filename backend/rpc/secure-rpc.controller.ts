...
export class SecureRpcController {
  ...
  constructor(
    ...
    this.enableSecureEndpoints = this.configService.get('ENABLE_SECURE_ENDPOINTS') === 'true';
    this.logger.log(`Secure RPC endpoints ${this.enableSecureEndpoints ? 'ENABLED' : 'DISABLED'}`);
  }

  @Post('mainnet')
  @ApiOperation({ summary: 'Proxy RPC calls to Solana Mainnet with security monitoring' })
  ...
  async proxyMainnetRpc(
    ...
    return this.secureRpcService.proxyRpcCall(apiKey, rpcPayload, userIp, userAgent);
  }

  @Post('/')
  @ApiOperation({ summary: 'Default RPC endpoint (Mainnet) with security monitoring' })
  @ApiHeader({ name: 'x-api-key', description: 'API key for authentication' })
  ...
  @ApiResponse({ status: 200, description: 'Node health status with security monitoring info' })
  async getHealth() {
  ...
  async getEndpoints() {
    return {
      mainnet: {
        ...
        description: 'Solana Mainnet RPC endpoint with enhanced security monitoring',
      ...
      original: {
        url: '/rpc',
        network: 'mainnet-beta', 
        description: 'Original RPC endpoint (unchanged for backward compatibility)',
