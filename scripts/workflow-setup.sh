...
# 1. backend INTEGRATION SCRIPT
cat > .workflow/get-backend-context.sh << 'EOF'
#!/bin/bash
# Get comprehensive backend context for debugging

echo "📊 backend Context Report"
echo "========================"
echo ""
...
backend variables --kv | head -20

echo ""
echo "📜 Recent Logs (Last 50 lines):"
backend logs 2>&1 | tail -50

echo ""
echo "❌ Recent Errors:"
backend logs 2>&1 | grep -E "(ERROR|error|Error|failed|Failed)" | tail -20

echo ""
echo "📈 Service Health:"
curl -s https://api.n0de.pro/health | jq . 2>/dev/null || echo "Service not responding"

echo ""
echo "🔗 Quick Links:"
echo "Build Logs: https://backend.com/project/262d4f31-c5ec-4614-8db6-b62bdb18ee17"
echo "Service Dashboard: https://backend.com/project/262d4f31-c5ec-4614-8db6-b62bdb18ee17/service/00deb85f-5cad-4617-99e7-0d7f00ac2be1"
EOF
...
CURRENT_HEALTH=$(curl -s https://api.n0de.pro/health 2>/dev/null || echo '{}')
echo "Current backend state recorded"

# Deploy
echo ""
echo "3️⃣ Deploying to backend..."
DEPLOY_OUTPUT=$(backend up --detach 2>&1)
echo "$DEPLOY_OUTPUT"
...
for i in {1..30}; do
  ...
  health_response=$(curl -s -o /dev/null -w "%{http_code}" https://api.n0de.pro/health 2>/dev/null)
  ...
  if [ "$health_response" = "200" ]; then
    ...
    backend logs 2>&1 | grep -E "(ERROR|error)" | tail -5
  fi
...
./.workflow/get-backend-context.sh
EOF
...
if echo "$LOGS" | grep -q "Can't reach database\|connection refused"; then
  ...
  echo "   Issue: Trying to connect to backend internal DB from outside"
  echo "   Fix: Remove local .env file, let backend use internal DATABASE_URL"
fi
...
echo ""
echo "🔗 backend Dashboard: https://backend.com/project/262d4f31-c5ec-4614-8db6-b62bdb18ee17"
EOF
...
echo "• ./.workflow/get-backend-context.sh - Get complete context for debugging"
...
echo "=============================="
echo ""
echo "1️⃣ **GIVE ME backend API TOKEN**:"
echo "   • Go to backend dashboard > Account Settings > Tokens"
echo "   • Create project token for this project"
echo "   • Set: export backend_TOKEN='your-token'"
echo "   • This gives me direct API access to logs, deployments, variables"
echo ""
echo "2️⃣ **ENABLE MCP SERVERS**:"
echo "   • Install backend MCP: pip install backend-mcp-server"
...
echo "   • Run: ./.workflow/get-backend-context.sh"
