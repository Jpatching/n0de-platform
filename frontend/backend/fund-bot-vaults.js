...
const DEPOSIT_AMOUNT = 0.5; // Deposit 0.5 SOL to each session vault

async function fundBotVaults() {
  console.log('💰 Funding bot session vaults...\n');
  ...
  for (const [botId, privateKeyArray] of Object.entries(BOT_PRIVATE_KEYS)) {
    try {
      ...
      if (walletBalance < DEPOSIT_AMOUNT * LAMPORTS_PER_SOL + 0.01 * LAMPORTS_PER_SOL) {
        console.log(`  ❌ Insufficient wallet balance for deposit + fees`);
        continue;
      }
      ...
      // Create a simple transfer transaction to fund the session vault
      // Note: This is a direct transfer, not using the PV3 program deposit function
      // The actual deposit should be done through the PV3 backend API
      console.log(`  ⚠️  Manual deposit required through PV3 backend API`);
      console.log(`  💡 Use: POST /api/v1/matches/session-vault/deposit with wallet signature`);
  ...
  console.log('🔧 To fund session vaults, each bot needs to:');
  console.log('1. Create session vault via: POST /api/v1/auth/create-session-vault');
  console.log('2. Deposit SOL via: POST /api/v1/matches/session-vault/deposit');
  console.log('3. The backend will handle the PV3 program calls');
}

fundBotVaults().catch(console.error); 