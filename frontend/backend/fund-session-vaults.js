...
async function fundSessionVaults() {
  console.log('💰 Funding bot session vaults via PV3 API...\n');
  ...
  for (const [botId, privateKeyArray] of Object.entries(BOT_PRIVATE_KEYS)) {
    try {
      ...
      // Step 1: Create session vault if it doesn't exist
      console.log('  📦 Creating session vault...');
      try {
        const createResponse = await fetch(`${API_BASE}/auth/create-session-vault`, {
          method: 'POST',
          headers: {
        ...
        if (createResponse.ok) {
          ...
          console.log(`  ✅ Session vault created: ${createResult.sessionVaultAddress}`);
        } else {
          console.log(`  ℹ️ Session vault already exists or creation failed`);
        }
      } catch (error) {
        console.log(`  ⚠️ Session vault creation error: ${error.message}`);
      }
      
      // Step 2: Check current balance
      console.log('  🔍 Checking current session vault balance...');
      try {
        const balanceResponse = await fetch(`${API_BASE}/matches/session-vault/${walletAddress}`);
      ...
      // Step 3: Fund the session vault (this requires the Solana program call)
      console.log(`  💸 This bot needs ${DEPOSIT_AMOUNT} SOL deposited to session vault`);
      console.log(`  🔧 Manual deposit required through frontend or direct Solana transaction`);
  ...
  console.log('🎯 Summary:');
  console.log('- All bots have session vaults created');
  console.log('- Bots need SOL deposited to their session vaults to join matches');
  console.log('- Use the frontend deposit feature or direct Solana transactions');
  console.log('- Once funded, bots should be able to join matches');
}

fundSessionVaults().catch(console.error); 