...
export default function AuthModal({ isOpen, onClose, mode, onModeChange }: AuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/${mode === 'signin' ? 'login' : 'register'}`, {
        method: 'POST',
        headers: {
          ...
          >
            ...
            <div className="relative mb-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-bg-elevated text-text-secondary">or</span>
              </div>
            </div>
              ...
              <div>
                ...
                {mode === 'signup' && password && (
                  <div className="mt-2 space-y-1">
                    <div className="text-xs text-text-secondary">Password requirements:</div>
                    <div className="grid grid-cols-2 gap-1 text-xs">
                      <div className={`flex items-center space-x-1 ${passwordStrength.isLongEnough ? 'text-green-400' : 'text-text-muted'}`}>
                        <div className={`w-2 h-2 rounded-full ${passwordStrength.isLongEnough ? 'bg-green-400' : 'bg-gray-600'}`} />
                        <span>8+ chars</span>
                      </div>
                      <div className={`flex items-center space-x-1 ${passwordStrength.hasUppercase ? 'text-green-400' : 'text-text-muted'}`}>
                        <div className={`w-2 h-2 rounded-full ${passwordStrength.hasUppercase ? 'bg-green-400' : 'bg-gray-600'}`} />
                        <span>Uppercase</span>
                      </div>
                      <div className={`flex items-center space-x-1 ${passwordStrength.hasLowercase ? 'text-green-400' : 'text-text-muted'}`}>
                        <div className={`w-2 h-2 rounded-full ${passwordStrength.hasLowercase ? 'bg-green-400' : 'bg-gray-600'}`} />
                        <span>Lowercase</span>
                      </div>
                      <div className={`flex items-center space-x-1 ${passwordStrength.hasNumber ? 'text-green-400' : 'text-text-muted'}`}>
                        <div className={`w-2 h-2 rounded-full ${passwordStrength.hasNumber ? 'bg-green-400' : 'bg-gray-600'}`} />
                        <span>Number</span>
                      </div>
                      <div className={`flex items-center space-x-1 ${passwordStrength.hasSpecial ? 'text-green-400' : 'text-text-muted'}`}>
            ...
            {mode === 'signup' && (
              <div className="mt-6 text-center text-sm text-text-secondary">
                ...
                <a href="#" className="text-N0DE-cyan hover:text-N0DE-sky transition-colors">
                  Terms of Service
                </a>{' '}
                and{' '}
                <a href="#" className="text-N0DE-cyan hover:text-N0DE-sky transition-colors">
