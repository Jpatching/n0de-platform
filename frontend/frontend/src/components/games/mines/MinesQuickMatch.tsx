...
interface MinesQuickMatchProps {
...
export default function MinesQuickMatch({ 
  onMatchFound, 
  onMatchCreated, 
  socketConnected, 
  gamePhase 
}: MinesQuickMatchProps) {
  const { user } = useAuth();
  ...
  const handleQuickSearch = async () => {
    ...
    try {
      ...
      const allMatches = await matchService.getAvailableMinesMatches();
      ...
      if (filteredMatches.length > 0) {
        ...
        const match = await matchService.createMinesMatch(optimalBet);
        setTimeout(() => {
  ...
  const handleAutoJoin = async () => {
    ...
    try {
      ...
      if (allMatches.length > 0) {
        ...
        const match = await matchService.createMinesMatch(filters.selectedBets[0]);
        setTimeout(() => {
          onMatchCreated(match);
          setIsSearching(false);
        }, 800);
