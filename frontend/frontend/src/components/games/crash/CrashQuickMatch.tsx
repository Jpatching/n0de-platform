...
interface CrashQuickMatchProps {
...
export default function CrashQuickMatch({ 
  onMatchFound, 
  onMatchCreated, 
  socketConnected, 
  gamePhase 
}: CrashQuickMatchProps) {
  const { user } = useAuth();
  ...
  const handleQuickSearch = async () => {
    ...
    try {
      ...
      const allMatches = await matchService.getAvailableCrashMatches();
      ...
      if (filteredMatches.length > 0) {
        ...
        const match = await matchService.createCrashMatch(optimalBet);
        setTimeout(() => {
  ...
  const handleAutoJoin = async () => {
    ...
    try {
      ...
      if (allMatches.length > 0) {
        ...
        const match = await matchService.createCrashMatch(filters.selectedBets[0]);
        setTimeout(() => {
          onMatchCreated(match);
          setIsSearching(false);
        }, 800);
