...
interface ChessQuickMatchProps {
...
export default function ChessQuickMatch({ 
  onMatchFound, 
  onMatchCreated, 
  socketConnected, 
  gamePhase 
}: ChessQuickMatchProps) {
  const { user } = useAuth();
  ...
  const handleQuickSearch = async () => {
    ...
    try {
      ...
      const allMatches = await matchService.getAvailableChessMatches();
      ...
      if (filteredMatches.length > 0) {
        ...
        const match = await matchService.createChessMatch(optimalBet);
        setTimeout(() => {
  ...
  const handleAutoJoin = async () => {
    ...
    try {
      ...
      if (allMatches.length > 0) {
        ...
        const match = await matchService.createChessMatch(filters.selectedBets[0]);
        setTimeout(() => {
          onMatchCreated(match);
          setIsSearching(false);
        }, 800);
