...
interface CoinflipQuickMatchProps {
...
export default function CoinflipQuickMatch({ 
  onMatchFound, 
  onMatchCreated, 
  socketConnected, 
  gamePhase 
}: CoinflipQuickMatchProps) {
  const { user } = useAuth();
  ...
  const handleQuickSearch = async () => {
    ...
    try {
      ...
      const allMatches = await matchService.getAvailableCoinFlipMatches();
      ...
      if (filteredMatches.length > 0) {
        ...
        const match = await matchService.createCoinFlipMatch(optimalBet);
        setTimeout(() => {
  ...
  const handleAutoJoin = async () => {
    ...
    try {
      ...
      if (allMatches.length > 0) {
        ...
        const match = await matchService.createCoinFlipMatch(filters.selectedBets[0]);
        setTimeout(() => {
          onMatchCreated(match);
          setIsSearching(false);
        }, 800);
