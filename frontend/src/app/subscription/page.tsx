...
const SubscriptionManagementPage = () => {
  ...
  const loadSubscriptionData = async () => {
    try {
      ...
      if (subscriptionData.status === 'fulfilled') {
        ...
        console.warn('Failed to load subscription data:', subscriptionData.reason);
      }
      ...
      if (usageData.status === 'fulfilled' && usageData.value) {
        ...
        setUsage({
          currentRequests: data.currentRequests ?? data.usage?.requests?.used ?? 0,
          requestLimit: data.requestLimit ?? data.usage?.requests?.limit ?? 1000000,
          currentKeys: data.currentKeys ?? data.usage?.apiKeys?.used ?? 0,
          keyLimit: data.keyLimit ?? data.usage?.apiKeys?.limit ?? 10,
          requestsThisMonth: data.requestsThisMonth ?? data.currentRequests ?? 0,
          lastRequestDate: data.lastRequestDate ?? '',
          resetDate: data.resetDate ?? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        });
      } else {
        console.warn('Failed to load usage data:', usageData.reason);
        ...
        setPlans([
          {
            ...
            limits: { requests: 1000000, apiKeys: 5, rateLimit: 1000, networks: ['mainnet'] }
          },
          {
            id: 'professional',
            name: 'Professional',
            ...
            limits: { requests: 10000000, apiKeys: 50, rateLimit: 5000, networks: ['mainnet', 'devnet'] },
            popular: true
          },
          {
            id: 'enterprise',
            ...
            interval: 'month',
            currency: 'USD',
            features: ['Unlimited requests', '24/7 support', 'Unlimited API keys'],
            limits: { requests: -1, apiKeys: -1, rateLimit: 50000, networks: ['mainnet', 'devnet', 'testnet'] },
            enterprise: true
          }
        ]);
      }
      ...
      if (subscriptionData.status === 'fulfilled' && subscriptionData.value?.status === 'active') {
        try {
          const methodsData = await api.get<PaymentMethod[]>('/payments/methods');
  ...
  const getUsagePercentage = (current: number | undefined, limit: number | undefined) => {
    if (current === undefined || current === null || !limit || limit === -1) return 0; // Unlimited or no data
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <CardDescription>
                            Active since {DatePresets.billing(subscription.created)}
                          </CardDescription>
                        </div>
                        <div className="text-right">
                          <div className="text-2xl font-bold text-text-primary">
                            {safeCurrency(subscription.amount, subscription.currency)}
                          </div>
                      ...
                      <div className="flex flex-wrap gap-3">
                        ...
                        {subscription.cancelAtPeriodEnd && (
                          <Badge variant="destructive">
                            ...
                            Cancels on {DatePresets.billing(subscription.currentPeriodEnd)}
                          </Badge>
                        )}
                      </div>
                      ...
                      <div className="grid md:grid-cols-2 gap-4 pt-4 border-t border-border">
                        <div>
                          ...
                          <p className="text-text-primary font-medium">
                            {DatePresets.billing(subscription.currentPeriodStart)} - {DatePresets.billing(subscription.currentPeriodEnd)}
                          </p>
                        </div>
                        <div>
                          <p className="text-sm text-text-secondary mb-1">Next Billing Date</p>
                          <p className="text-text-primary font-medium">
                            {subscription.cancelAtPeriodEnd ? 'Cancelled' : DatePresets.billing(subscription.currentPeriodEnd)}
                          </p>
                        </div>
                      </div>
                    </CardContent>
                ...
                >
                  <Card className="bg-bg-elevated border-border">
                    <CardHeader>
                      ...
                      <CardDescription>
                        {usage?.resetDate ? `Resets on ${DatePresets.billing(usage?.resetDate)}` : 'Usage data loading...'}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm text-text-primary">
                              {usage?.currentRequests?.toLocaleString() || '0'} 
                              {usage?.requestLimit === -1 ? ' / Unlimited' : ` / ${usage?.requestLimit?.toLocaleString() || '0'}`}
                            </span>
                          </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm text-text-primary">
                              {usage?.currentKeys || 0} 
                              {usage?.keyLimit === -1 ? ' / Unlimited' : ` / ${usage?.keyLimit || 0}`}
                            </span>
                          </div>
                          <div className="w-full bg-bg-hover rounded-full h-2">
                            <div 
                      ...
                      <div className="grid md:grid-cols-3 gap-4 mt-6 pt-6 border-t border-border">
                        <div className="text-center">
                          <div className="text-2xl font-bold text-N0DE-cyan">
                            {usage?.requestsThisMonth?.toLocaleString() || '0'}
                          </div>
                        ...
                        <div className="text-center">
                          <div className="text-2xl font-bold text-N0DE-purple">
                            {usage?.lastRequestDate ? DatePresets.activity(usage?.lastRequestDate) : 'Never'}
                          </div>
                        ...
                        {(paymentHistory || []).map((payment) => (
                          <div key={payment.id} className="flex items-center justify-between p-4 bg-bg-hover rounded-lg border border-border">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <div className="text-text-primary font-medium">
                                  {safeCurrency(payment.amount, payment.currency)}
                                </div>
                                <div className="text-sm text-text-secondary">
                                  {payment.description} • {DatePresets.payment(payment.date)}
                                </div>
                              </div>
                            </div>
                            <div className="flex items-center gap-2">
                ...
                <div className="bg-bg-hover p-4 rounded-lg">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-xl font-bold text-text-primary">
                      {safeCurrency(selectedPlan.price, selectedPlan.currency)}
                    </span>
                  </div>
